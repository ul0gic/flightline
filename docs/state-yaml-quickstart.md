...
```bash
fline fetch com.under5.passdmv -o state.yaml
```
...
```
fline: wrote state.yaml (2847 bytes)
```
...
`~` means update. `+` means create. `-` means delete. Read-only: `fline plan` never writes.

**JSON output for scripts:**

```bash
fline plan state.yaml --output json
```
...
Without `--confirm`, `fline apply` prints the plan and refuses to write — the safety gate. This matches Terraform's behavior: `apply` without `--confirm` is equivalent to `plan`.

Expected stderr output during apply:

```
fline: applied ~ /spec/metadata/locales/en-US/promotionalText
```
...
|---------|-------------|---------|
| `fline fetch <bundleId>` | Snapshot live ASC state into YAML | No |
| `fline fetch <bundleId> --version 1.0` | Fetch a specific version | No |
| `fline fetch <bundleId> --output json` | Fetch as JSON (for piping, not editing) | No |
| `fline plan state.yaml` | Preview what would change | No |
| `fline plan state.yaml --exit-on-changes` | Plan + CI gate exit code | No |
| `fline plan state.yaml --output json` | Machine-readable diff | No |
| `fline apply state.yaml` | Plan only, refuses to write | No |
| `fline apply state.yaml --confirm` | Write all changes | Yes |
| `fline apply state.yaml --confirm --resume` | Resume after interruption | Yes |
| `fline apply state.yaml --dry-run` | Compute dispatch, no wire calls | No |
...
2. Lint       fline lint state.yaml           # Phase 5 — offline schema + format rules
3. Plan       fline plan state.yaml           # read-only diff, no writes
4. Apply      fline apply state.yaml --confirm
5. Preflight  fline preflight <bundleId> -v <v>  # Phase 5 — live rule check
6. Submit     fline submit <bundleId> -v <v>  # triggers Apple Review
```
...
`fline apply` cannot yet drive binary asset uploads (screenshots, IAP review screenshots, Custom Product Page screenshots). The diff engine compares checksums correctly, and `fline plan` shows what would change, but `fline apply` returns a typed error for these paths and directs you to the L1 verbs:

```bash
# Upload screenshots directly via L1
fline screenshots upload com.under5.passdmv \
...
`fline privacy-labels get <bundleId>` returns a diagnostic explaining this gap — it does not fail silently. See [ISSUE-002](../.project/issues/closed/ISSUE-002-privacy-labels-not-in-asc-api.md).
...
The version in `metadata.version` doesn't exist in ASC yet. Either create the version first (`fline versions create`) or update `metadata.version` to match an existing editable version.
